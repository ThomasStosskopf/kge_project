...
class PrepareData():
    ...
    def load_data(self)->Tuple[KnowledgeGraph, KnowledgeGraph, KnowledgeGraph]:
        ...
        df1 = read_csv('data/train_set.csv',
                    sep=',', names=['from', 'rel', 'to'])
        df2 = read_csv('data/val_set.csv',
                    sep=',', names=['from', 'rel', 'to'])
        df3 = read_csv('data/test_set.csv',
                    sep=',', names=['from', 'rel', 'to'])
    ...
    def encode_relations_and_save(self, path:str)->DataFrame:
        ...
        relation_df.to_csv("data/relation_id.csv", index=False)
    ...
    def main(self)->None:
        ...
        train_data.to_csv('data/train_set.csv', index=True,columns=['from', 'rel', 'to'])
        val_data.to_csv('data/val_set.csv', index = False, columns=['from', 'rel', 'to'])
        test_data.to_csv('data/test_set.csv', index=False, columns=['from', 'rel', 'to'])
...
if __name__ == "__main__":
    ...
    prepare_data = PrepareData('data/KG_edgelist_mask.txt')
